...
このリポジトリの `plugins-sample/` （標準同梱の5機能＋`reading-goal`）も同じ方法でインストールできます。例: `https://github.com/hahero-asayake/bookshelf/tree/main/plugins-sample/author-grouping`

一通りの拡張点（コマンド・ボタン・CSS 注入・イベント購読）を最小コードで使う例:
...
{
    "id": "hello-bookshelf",
    "name": "Hello Bookshelf",
    "version": "0.2.0",
    "description": "新プラグインAPIの最小サンプル。registerCommand / addUIButton / injectCSS / イベント購読を一通り使う。",
    "icon": "hand",
    "publishable": false,
    "categories": ["command", "button"],
    "files": ["index.js"],
    "dependencies": []
}
```
