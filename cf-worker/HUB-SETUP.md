...
- **実績 (#213・2026-09-25・OGP 画像 `og.png` の base64 公開経路)**: hub `c29766d0-0fc5-4938-9974-6a224983ffbf` (HEAD=e08772c・**hub のみ deploy。bookshelf-cdn は未変更**)。直前版は hub `0da945c5-ab86-4111-9ad5-f904043a09d7` (#210 の版)。rollback は `wrangler rollback 0da945c5-ab86-4111-9ad5-f904043a09d7 -c wrangler.hub.toml -m <理由>`。⚠️ worker だけ戻すとアプリ (main) が送る base64 PNG を旧 worker が文字列として計量し公開が壊れる＝戻す時は main の revert/公開停止とセットにする。疎通確認は本番で `POST /publish` を 従来の文字列のみ / base64 PNG / `.png` 以外への base64 (400) / PNG でないバイト列 (400) の 4 通りで確認済み (quota は復号後バイト長で計量: 本番 `usedBytes` 121040 = 記事 HTML 8769 + トップ 6896 + og.png 105375 の一致を実測)。
- **実績 (#220・2026-09-26・索引屋 = 索引登録/削除連動/通報・ADR-099)**: hub `ae537d89-aca0-4375-9616-02d455d4d547` (HEAD=96bacb4・**hub のみ deploy。bookshelf-cdn は未変更**)。順序は ①`d1 export` を bookshelf/tmp へ退避 (`asayake-community-before-2026-09-25.sql`・gitignore 済み) → ②D1 マイグレーション `0001_index_articles.sql` を適用 (適用前 = sites 0 件・reports 1 件・旧列のみ。0002 は sites の `public_id=''` が 0 件のため不要) → ③専用 Discord webhook を #📚bookshelf に作成しテスト投稿 1 回 (届くことを確認) → ④`secret put REPORT_WEBHOOK_URL` (これで **Worker の版が 1 つ増える**: `cfa76361-38de-4a2e-8d91-28e7386c0293` = 旧コード＋secret 込みの中間版) → ⑤hub deploy → ⑥curl → ⑦bookshelf を push。直前版は hub `c29766d0-0fc5-4938-9974-6a224983ffbf` (#213 の版・secret 無し)。rollback は `wrangler rollback c29766d0-0fc5-4938-9974-6a224983ffbf -c wrangler.hub.toml -m <理由>` (通知を残したいなら secret 込みの `cfa76361-…`)。**D1 は追加型なので旧 Worker のまま動く** (マイグレーション後・deploy 前に旧コードの `GET /community/sites`=200 を確認済み)。ただし deploy 後に退会 (墓標化) が走った後の rollback は旧コードが墓標に 409 を返すため自分で戻さず判断する。
