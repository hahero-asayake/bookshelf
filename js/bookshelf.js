...
お気に入り、メモ、本棚からも削除されます。`;
     ...
     */
    /**
     * Amazon 蔵書一覧ページに一意 id (bs-import-progress) のオーバーレイを出し、
     * 起動→取得中(n/total)→送信中→完了/失敗 を表示する (イシュー#89)。
     * <style> 注入・setAttribute('style',…) は使わず style.cssText で当てる (CSP の style-src-attr 対象外)。
     * 呼び直すたびに同 id の古いパネルを除去する = 進捗表示は最終結果パネルに置き換わって残らない。
     * 完了/失敗のうちユーザーが読む必要のある内容 (clipboard 完了・失敗理由・csrfToken 不在) だけ閉じるボタンを付ける。
     */
    _buildKindleBookmarkletPanelCode() {
        return `var bp=function(msg,isErr,closable){var o=document.getElementById('bs-import-progress');if(o)o.remove();var v=document.createElement('div');v.id='bs-import-progress';v.style.cssText='position:fixed;inset:0;z-index:2147483647;background:rgba(15,23,42,.9);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;padding:24px;font-family:sans-serif';var p=document.createElement('div');p.style.cssText='color:'+(isErr?'#fca5a5':'#fff')+';font-size:17px;line-height:1.7;text-align:center;max-width:520px;word-break:break-word';p.textContent=msg;v.appendChild(p);if(closable){var x=document.createElement('button');x.textContent='閉じる';x.style.cssText='font-size:14px;padding:10px 24px;border:0;border-radius:8px;background:#475569;color:#fff;cursor:pointer';x.onclick=function(){v.remove();};v.appendChild(x);}document.body.appendChild(v);return p;};var bu=function(el,msg){if(el&&el.isConnected)el.textContent=msg;};`;
    }
