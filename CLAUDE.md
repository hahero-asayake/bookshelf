...
| イベント追加・削除 | 02_基本設計書 + 05_プラグイン仕様書 (イベント一覧) |
...
未公開アプリのため、既存データとの互換性は考慮しない (旧形式マイグレーションを書く必要なし)。
...
| Phase 2 (本棚・継承) | ✅ |
| Phase 3 (公開エクスポート) | ✅ |
| Phase 4 (ワンクリック Kindle 取込) | ❌ 未着手 |
| Phase 5 (プラグイン基盤) | ✅ |
| **段階1: StorageAdapter 抽象化 (リファクタ)** | ⏳ コード完了、動作確認待ち (2026-05-30) |
| **段階2-1: GitHubAdapter コア (単一 PUT, sha キャッシュ)** | ⏳ コード完了、動作確認待ち (2026-05-30) |
| **段階2-2: 設定 UI + 起動フロー分岐** | ⏳ コード完了、動作確認待ち (2026-05-30) |
| **段階2-3: Trees API バッチ + 楽観ロック衝突検知** | ⏳ コード完了、動作確認待ち (2026-05-31) |
| **段階2-4: plugin-loader + 他機能の adapter 化** | ⏳ コード完了、動作確認待ち (2026-05-31) |
| **R-1: ストレージ構造を `private/` 配下に集約** | ⏳ コード完了、動作確認待ち (2026-05-31) |
| **R-2: 公開エクスポート簡素化 (`public/` 直書き, 1 commit)** | ⏳ コード完了、動作確認待ち (2026-05-31) |
| **R-3: 長文メモエディタ (EasyMDE) + 開き方設定** | ⏳ コード完了、動作確認待ち (2026-05-31) |
| **段階3: PC UI 刷新 (UI-1〜UI-5)** | ✅ 2026-05-31 (3 ペインシェル + 本棚ツリー + 詳細ペイン + 折りたたみ + ダッシュボード実装、UI-6/7 未着手) |
| **段階3: Google Drive / Dropbox Adapter** | ❌ 未着手 |
| Lucide アイコン化 (任意文字対応 + 全アイテムカスタマイズ) | ✅ 2026-05-31 |
| **大改修 Phase A** (絵文字全廃 + CSS 変数統一 + btn-primary 統一 + 旧 modal 削除) | ✅ 2026-06-01 |
| **大改修 Phase B** (本詳細ペイン書き直し + メモ仕様再設計 + チェックボックス stylish 化 + B-9 星/メモ表示を全体設定化 (starVisibility/starOverlay/memoVisibility)・一覧クリック評価) | ✅ 2026-06-01 |
| **大改修 Phase C** (ヘッダー最小化 brand+⌘K+設定) | ✅ 2026-06-01 |
| **大改修 Phase D** (⌘K コマンドパレット本体) | ✅ 2026-06-01 |
| **大改修 Phase E** (本棚 toolbar 化 + 複数選択/一括操作) | ✅ 2026-06-01 |
| **大改修 Phase G** (本棚編集をサイドバーツリーから) | ✅ 2026-06-01 |
