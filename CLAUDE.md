...
- 短文メモ (2026-06-20 改訂, ADR-007): **ALL 1段のみ** (本棚 override は廃止)。`notes.json[asin].memo` が唯一の正本で、どの本棚・ホームから開いても同じ。閲覧モードでも直接編集。本棚別メモが要る場合は**プラグイン**で疎結合に加算 (ADR-041)。
  - 公開時: `hideMemo` フラグで opt-out (default 公開 ON)
  - `resolveMemo(asin)` / `setMemo(asin, memo)` (第2引数の本棚スコープは廃止)
...
### commit 前の確認 (事故防止・イシュー#11 教訓)

**`git add` で自分が変更したファイルを明示指定しても、他の作業が既にステージ済みなら commit はそれも巻き込む**（`git add` は「追加」であって「ステージ内容の置き換え」ではないため）。イシュー#11 で実際に事故りかけた（無関係な26ファイルを巻き込んで commit → push 前に検知して復旧）。

- **commit 前に必ず `git diff --cached --stat` で、ステージされている内容が自分の変更だけかを確認する**
- **`git add -A` / `git commit -a` は使わない**（無関係な変更を無条件に巻き込むため、常に個別ファイルを指定する）
- 他の作業が既にステージされているのを見つけたら、その場で判断せず先に本人に確認する（勝手に unstage しない・勝手に含めて commit しない）

### commit 実行の承認 (2026-08-16)

**commit すること自体に本人承認は要らない**（本人「commit は kuroko の判断で勝手にして良い」）。粒度とタイミングはクロコ／イシュー実行役が決める＝承認待ちで作業ツリーを未コミットのまま放置しない。push も bookshelf 本体は未公開のため自由（公開後は承認側へ移す）。**本番デプロイ (`wrangler deploy`) は、②のレビュー承認後に kuroko が範囲を絞った Cloudflare API トークンで実行する**（2026-09-24 のハヘロ決裁で、8/16 の「本人手動・トークンを置かない」方針を変更。トークンの保存場所は `/home/kuroko/.local/share/kuroko/cloudflare-token`＝パスのみ記載・値はリポジトリ/ログ/報告に一切出さない。渡し方・rollback は `cf-worker/HUB-SETUP.md` の「デプロイ運用」）。上の「commit 前の確認」は承認とは別の事故防止なので引き続き守る。
...
bookshelf は **StorageAdapter** で 3 方式を切替できる:
