...
**commit すること自体に本人承認は要らない**（本人「commit は kuroko の判断で勝手にして良い」）。粒度とタイミングはクロコ／イシュー実行役が決める＝承認待ちで作業ツリーを未コミットのまま放置しない。push も bookshelf 本体は未公開のため自由（公開後は承認側へ移す）。**本番デプロイ (`wrangler deploy`) だけは本人手動が正**（この環境に API トークンを置かない方針）。上の「commit 前の確認」は承認とは別の事故防止なので引き続き守る。
